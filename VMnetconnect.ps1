...
$Form.Text = "Connect VMs to Network"
...
$LabelNetwork = Create-Control -text "Select VM Network:" -x 30 -y 410 -width 150 -height 20 -type "Label" -form $Form
$ComboBoxNetwork = Create-Control -x 180 -y 410 -width 200 -height 20 -type "ComboBox" -form $Form
$ButtonConnectNetwork = Create-Control -text "Connect to Network" -x 180 -y 450 -width 150 -height 30 -type "Button" -form $Form
...
# Function to publish VM network dropdown
function Publish-VMNetworkDropdown {
    param (
        $networks,
        $comboBox
    )
    ...
    foreach ($network in $networks) {
        $null = $comboBox.Items.Add($network)
    }
...
$ButtonConnect.Add_Click({
    ...
    if ($vcHost -eq "" -or $vCenterUsername -eq "" -or $vCenterPassword -eq "") {
        ...
        if ($connected) {
            ...
            $ButtonConnectNetwork.Enabled = $true
            ...
            # Publish the VM network dropdown
            $networks = Get-VirtualNetwork | Sort-Object Name
            Publish-VMNetworkDropdown -networks $networks -comboBox $ComboBoxNetwork
        }
        else {
...
# Event handler for Connect to Network button
$ButtonConnectNetwork.Add_Click({
    $selectedVMs = $ListBoxVMs.SelectedItems
    $selectedNetwork = $ComboBoxNetwork.SelectedItem

    if ($selectedVMs.Count -eq 0) {
        $StatusLabel.Text = "Please select VM(s) to connect to the network."
        $StatusLabel.ForeColor = "Red"
    }
    elseif (-not $selectedNetwork) {
        $StatusLabel.Text = "Please select a network to connect the VMs."
        $StatusLabel.ForeColor = "Red"
    }
    else {
        Write-Host "`nConnecting VMs to network..."
        $selectedVMs | ForEach-Object {
            $vm = $_
            try {
                # $NetSet = Set-NetworkAdapter -Connected $true -NetworkAdapter $NetAdapter -NetworkName $selectedNetwork
                $NetSet = Get-VM $vm | Get-NetworkAdapter | Set-NetworkAdapter  -RunAsync -NetworkName "$selectedNetwork" -StartConnected $true -Confirm:$false
                Write-Host "Connected VM '$vm' Network Adapter to network '$selectedNetwork'."
            }
            catch {
                Write-Host "Failed to connect VM '$vm' to network '$selectedNetwork'. $NetSet" -ForegroundColor Red
            }
        }

        $StatusLabel.Text = "VM(s) connected to the network successfully."
        $StatusLabel.ForeColor = "Green"
    }
})
